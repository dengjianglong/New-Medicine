...
<li><a href="${pageContext.request.contextPath}/dm/findAllDM.do">表1  t_dm</a></li>
        ...
        <form action="${pageContext.request.contextPath}/prescription/savePrescription.do" methon="post">
        
         <span class="span1">Nane</span>
        <input id="xm" type="text" name="pPrescription"/>
        <span class="span1">Attribute</span>
        <input id="ch" type="text" name="pFunction"/>
        <span class="span1">Attribute</span>
        <input id="d" type="text" name="pCute"/>
        <span class="span1">Attribute</span>
        <input id="e" type="text" name="pClinical"/>
        <span class="span1">Attribute</span>
        <input id="f" type="text" name="pSolution"/>
        <span class="span1">Attribute</span>
        <input id="g" type="text" name="pUse"/>
 ...
 <button class="but" type="submit">添加</button>
        ...
        <table id="mytable" cellspacing="0px" border="3" bordercolor="red">
            ...
            <c:forEach items="${prescription }" var="prescription">
             <tr>
                <td class="s1">${prescription.id }</td>
                <td class="s2">${prescription.pPrescription }</td>
                <td class="s3">${prescription.pFunction }</td>
                <td class="s4">${prescription.pCute }</td>
                <td class="s5">${prescription.pClinical }</td>
                <td class="s6">${prescription.pSolution }</td>
                <td class="s7">${prescription.pUse }</td>
                <td class="s9"><a href="${pageContext.request.contextPath}/prescription/deletePrescription.do?id=${prescription.id}" onclick='Del(this.parentNode,this.parentNode.parentNode.rowIndex)'>删除</a></td>
